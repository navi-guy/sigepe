...
class RevisarStockController extends Controller
{
    ...
    {
        $insumos = Insumo::groupBy(self::INSUMOS_ID)->join('insumos_proveedor', 'insumos_proveedor.insumo_id', '=', self::INSUMOS_ID)->selectRaw('insumos.id, insumos.nombre, insumos.cantidad, insumos.unidad_medida, insumos_proveedor.insumo_id, MAX(insumos_proveedor.estado) AS estado, SUM(insumos_proveedor.cantidad) AS solicitado')->orderBy(self::INSUMOS_ID, 'DESC')->get();
